...
Let's have a look at the distribution of the response variable (the red dashed line shows the true number of non-Western immigrants):
...
response_count <- data_raw %>%
  dplyr::group_by(imm_brit) %>%
  dplyr::summarise(no_imm_count = n()) %>%
  data.frame()

nw_immi <- 100 * (6.973 / 65.14)  # True number of non-western immigrants in the UK
...
bar_plot <- ggplot(data = response_count,
  ...
  scale_x_continuous(name = "Estimated number of non-Western immigrants") +
