...
bsas_data <- bsas_data %>%
  dplyr::mutate(resp_urban_area = factor(resp_urban_area,
                                         levels = c("rural", "rather rural",
                                                    "rather urban", "urban")),
                resp_health = factor(resp_health, levels = c("bad", "fair",
                                                             "fairly good", "good")))

# Standardize quantitative predictors
cat_vars <- unlist(lapply(bsas_data, function(x) is.factor(x) | all(x == 0 | x == 1, na.rm = TRUE)))
bsas_data[, !cat_vars] <- apply(bsas_data[, !cat_vars], 2, scale)
```
